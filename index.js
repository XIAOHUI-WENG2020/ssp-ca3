const   http = require('http'), //provides the HTTP server functionalities
        ...
        express = require('express'), //allows app to respond to HTTP requests, defines the routing and renders back the required content
...
router.use(express.urlencoded({extended: true})); //the data sent from the client to be encoded in a URL targeting our end point
...
router.get('/get/html', function(req, res) {
    ...
    let xml = fs.readFileSync('joblist.xml', 'utf8'),
        xsl = fs.readFileSync('joblist.xsl', 'utf8');
...
router.post('/post/json', function (req, res) {
    ...
    function appendJSON(obj) {
        ...
        XMLtoJSON('joblist.xml', function (err, result) {
            ...
            result.joblist.jobType[obj.sec_n].entry.push({'company': obj.company, 'jobTitle': obj.jobTitle});

            console.log(JSON.stringify(result, null, "  "));

            JSONtoXML('joblist.xml', result, function(err){
